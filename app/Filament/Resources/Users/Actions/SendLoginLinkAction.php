<?php

declare(strict_types=1);

namespace App\Filament\Resources\Users\Actions;

use App\Domains\User\Actions\SendLoginLink;
use App\Domains\User\Enums\PermissionEnum;
use App\Domains\User\Models\User;
use Exception;
use Filament\Actions\Action;
use Filament\Notifications\Notification;
use Filament\Support\Icons\Heroicon;

class SendLoginLinkAction extends Action
{
    public static function getDefaultName(): ?string
    {
        return 'sendLoginLink';
    }

    protected function setUp(): void
    {
        parent::setUp();

        $this->authorize(PermissionEnum::MANAGE_ALL)
            ->label('Send Login Link')
            ->color('info')
            ->outlined()
            ->visible(fn (User $record) => $record->is_local_user && config('auth.local.enabled'))
            ->icon(Heroicon::OutlinedPaperAirplane)
            ->requiresConfirmation()
            ->modalDescription('This will send a new login link to the user\'s email address.')
            ->action(function (User $record) {
                try {
                    resolve(SendLoginLink::class)($record);

                    Notification::make()
                        ->success()
                        ->title('Login link sent')
                        ->body('A new login link has been sent to ' . $record->email . '.')
                        ->send();
                } catch (Exception $e) {
                    Notification::make()
                        ->danger()
                        ->title('Failed to send login link')
                        ->body($e->getMessage())
                        ->send();
                }
            });
    }
}
