...
**See also:** [Idempotent Seeding](/architecture/03-idempotent-seeding/) documentation
...
**See also:** [Database Snapshots](/features/04-database-snapshots/) documentation
...
### `send:api-token-expiration-notifications`

Send expiration reminder emails for API tokens approaching their expiration date.

```bash
php artisan send:api-token-expiration-notifications
```

**What it does:**

1. Queries for active API tokens with upcoming expiration dates
2. Checks which tokens are due for notifications (30, 14, 7, 3, 1 days before)
3. Sends reminder emails to token owners

**See also:** [API Documentation](/features/03-api/#expiration-notifications)
