...
<Aside>
    The starter does **not** bake in deployment infrastructure. Teams differ in
    AWS permissions, hosting patterns, and operational constraints.
</Aside>
...
<Aside>
    Certificates **must** be created in `us-east-1` even if your Lambdas run in
    another region. API Gateway edge-optimized endpoints only read ACM
...
<Aside type="caution">
    If your application requires multi-minute synchronous operations or custom
    PHP extensions, Vapor may not be suitable.
</Aside>
...
<Aside type="danger">
    This workflow is a **reference implementation**. It includes branch-specific
    logic and environment variable patterns you may not need. Treat it as a
    starting point, not a drop-in solution.
</Aside>
