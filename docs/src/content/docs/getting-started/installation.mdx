...
<Steps>
    1. **Create Databases**
        ...
        Create the two databases required by the application. The steps depend on your chosen database driver:
        ...
        **SQLite**:
        ```bash
        touch database/database.sqlite   # Primary application database
        touch database/testing.sqlite     # PHPUnit test database
        ```
...
In your **LOCAL** `.env` file, configure credentials depending on your authentication strategy. See the [WebSSO / Entra ID](/northwestern-integrations/websso/) documentation for full details on each provider.

<Tabs>
  <TabItem label="Entra ID" icon="seti:lock">
  ...
  <TabItem label="Online Passport (WebSSO)" icon="seti:lock">
    ...
    <Aside type="tip">
    Setting `WEBSSO_API_KEY` automatically switches the starter from Entra ID to Online Passport. See the [WebSSO / Entra ID](/northwestern-integrations/websso/#online-passport-configuration) docs for additional configuration options like `WEBSSO_STRATEGY` and `DUO_ENABLED`.
    </Aside>
  </TabItem>
</Tabs>
