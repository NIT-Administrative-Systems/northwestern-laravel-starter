...
<Aside type="tip">
All rate limiters (API, authentication, support) are centralized in `config/rate-limiting.php` with env var overrides. See the [API](/features/api/#rate-limiting), [Authentication](/features/authentication/#rate-limiting), and [Support Tickets](/features/support-tickets/#rate-limiting) pages for details on each limiter.
</Aside>
...
<Aside type="tip">
Useful for testing or for enabling lockdown in production during a closed beta launch.
</Aside>
...
```diff title="config/platform.php"
