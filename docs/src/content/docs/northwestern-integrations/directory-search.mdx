...
1. **Search type detection**
   ...
   `DirectorySearchTypeEnum::fromSearchValue()` auto-detects whether the input is an email address, employee ID (numeric), or NetID (fallback).
...
| `DirectorySearchTypeEnum` | Auto-detects search input type (email, employee ID, NetID) |
...
After a user is successfully synced and persisted, `FindOrUpdateUserFromDirectory` dispatches a configurable list of **post-retrieval jobs**. This is the primary extension point for running custom logic whenever a user is fetched from the directory — whether during SSO login, manual creation in Filament, or a scheduled directory sync.
...
The `SyncUserFromDirectory::syncDemographics()` method defines how LDAP directory attributes map onto User model fields. This is the place to customize when your project needs different directory fields, additional attributes, or different priority logic.
...
3. **Update the User model** (if needed)
   ...
   Add a cast if the field requires one (e.g., `array`, `datetime`). No `$fillable` changes are needed — the starter runs `Model::unguard()` globally.
