...
Access tokens are stored as HMAC-SHA256 hashes derived from `APP_KEY`. If you rotate `APP_KEY`, **all existing API tokens will stop working** because their hashes will no longer match. Before rotating `APP_KEY`, coordinate with all API consumers to rotate their tokens immediately afterward using the administration panel.
</Aside>
