...
**Pending** - Token created but not yet active (before `valid_from` date)

**Active** - Token is active and can be used for authentication

**Expired** - Token has passed its `valid_to` date and cannot be used

**Revoked** - Token manually disabled
...
For high-volume APIs, consider sampling to reduce the database load:
...
:::tip[Failures Always Logged]
Non-successful requests (`4xx`/`5xx`) are always logged regardless of the sample rate.
:::
