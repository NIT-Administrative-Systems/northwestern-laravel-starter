...
The application uses `InMemoryHealthResultStore` by default because:

- Health check results are ephemeral and typically don't require historical persistence
- Notifications are the primary alerting mechanism
- Reduces database writes since checks run every minute

If historical tracking is needed for trend analysis or compliance, publish the migration with `php artisan vendor:publish --tag="health-migrations"` and switch to `EloquentHealthResultStore` in the configuration.
