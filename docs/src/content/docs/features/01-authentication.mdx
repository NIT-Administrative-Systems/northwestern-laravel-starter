...
The Northwestern Laravel Starter provides a flexible, multi-method authentication system designed to handle both Northwestern users (via NetID/SSO) and external collaborators (via passwordless login links or API tokens).
...
<CardGrid>
  ...
  <Card title="API Tokens" icon="rocket">
...
5. **Middleware validates**
   ...
   `AuthenticatesApiTokens` middleware validates token and IP allowlist
...
See the [API Documentation](/features/03-api/) for complete API token management details.
